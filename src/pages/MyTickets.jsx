...
function MyTickets({ currentUser }) {
  ...
  const filteredTickets = tickets.filter((ticket) => {
    ...
    if (selectedPriority !== "ALL LEVELS") {
      matchesPriority = priority === selectedPriority.toLowerCase();
    }
      ...
      <main className="main-content inbox-page-container">
        ...
        <div className="tickets-inbox-wrapper">
          {filteredTickets.length === 0 ? (
            <div className="dashboard-message">No tickets found.</div>
          ) : (
            <div className="ticket-grid-container">
              {filteredTickets.map((ticket, index) => {
                ...
                const creator = ticket.createdBy?.name || ticket.user?.name || 'Test Employee';
                ...
                return (
                  <div className="ticket-card-item" key={ticketId || index}>
                    <div className="ticket-card-top">
                      <div className="ticket-card-user-info">
                        <div className="ticket-card-avatar">👤</div>
                        <span className="ticket-card-id">#{shortId}</span>
                        <span className="ticket-card-count">({messageCount})</span>
                    ...
                    <div className="ticket-card-details">
                      <div><span className="detail-lbl">REQUESTER:</span> REQ: {creator}</div>
                      <div><span className="detail-lbl">ASSIGNED TO:</span> {assignedTo}</div>
                      <div><span className="detail-lbl">DEPARTMENT:</span> {department}</div>
                    </div>

                    <div className="ticket-card-footer">
                      <Link to={`/tickets/${ticketId}`} className="view-details-btn">
                        VIEW DETAILS
                      </Link>
                    </div>
                  </div>
